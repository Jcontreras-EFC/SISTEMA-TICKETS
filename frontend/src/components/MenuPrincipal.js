...
import BackupCorreos from './BackupCorreos';
...
const MenuPrincipal = ({ seccion, setSeccion, permisos, usuarioLogueado }) => {
  ...
  const panelSuperiorRef = useRef(null);
  ...
  useEffect(() => {
    if (!editando) {
      fetch('http://192.168.12.66:3001/api/tickets/next-numero')
        .then(res => res.json())
  ...
  const handleRegistrar = (nuevo) => {
    let ticketSeguro;
    if (editando) {
      // Si se está editando, mantener el estado original
      ticketSeguro = { ...nuevo, estado: editando.estado };
    } else {
      // Si es nuevo, estado por defecto 'abierto'
      const estadoSeguro = (nuevo.estado || 'abierto').toLowerCase();
      ticketSeguro = { ...nuevo, estado: estadoSeguro };
    }
    if (editando) {
      fetch(`http://192.168.12.66:3001/api/tickets/${editando.id}`, {
        method: 'PUT',
  ...
  const handleEditar = (ticket) => {
    if (ticket.estado && ticket.estado.toLowerCase() === 'cerrado') {
  ...
  const handleEliminar = (ticket) => {
      fetch(`http://192.168.12.66:3001/api/tickets/${ticket.id}`, {
        method: 'DELETE'
      })
  ...
  const handleLogout = () => {
    // Limpiar token o sesión (ajusta según tu lógica de autenticación)
  ...
  // --- RENDER PRINCIPAL ---
  return (
    <div className="min-h-screen bg-gray-50 w-full h-full flex-1">
      {/* Modal profesional para ticket cerrado */}
      {modalCerrado && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-2xl shadow-2xl p-8 border-2 border-red-400 max-w-sm w-full text-center animate-fade-in">
            <h3 className="text-2xl font-bold text-red-700 mb-4">Ticket cerrado</h3>
            <p className="mb-6 text-gray-700">No se puede editar un ticket que ya está <span className="font-bold">cerrado</span>.</p>
            <button className="bg-red-600 text-white px-8 py-2 rounded-lg font-semibold hover:bg-red-700 transition" onClick={() => setModalCerrado(false)}>Aceptar</button>
          </div>
        </div>
      )}
      {/* El resto de la lógica de renderizado según sección */}
      {seccion === 'dashboard' && <DashboardReportes />}
      {seccion === 'tickets' && (
        <main className="flex-1 w-full min-h-screen overflow-auto flex flex-col bg-gray-100">
          <div className="bg-white w-full h-full flex-1" style={{minHeight:'calc(100vh - 64px)'}}>
            <FormularioTicket
        ...
        >
          ...
          <div className="flex flex-col items-center w-full" style={{width: '100%'}}>
            <TicketsRecientes
              ...
              onEditarSuperior={handleEditarSuperior}
              onEliminarSuperior={handleEliminarSuperior}
              panelSuperiorHeightRef={panelSuperiorRef}
            />
